...
static	void	ft_feed_line(t_list **p, char **line, int l)
{
	char	*tmp;

	*line = ft_strdup((*p)->content);
	tmp = (*p)->content;
	(*p)->content = ((l > 0) ? ft_strdup((*p)->content + l) : ft_strdup(""));
	ft_strdel(&tmp);
	return ;
}
...
int				get_next_line(const int fd, char **line)
{
	...
	{
		buff[ret] = '\0';
		ptr->content = ft_strjoinfree(ptr->content, buff, 1);
	}
	if (len > 0)
		((char*)ptr->content)[len - 1] = '\0';
	ft_feed_line(&ptr, line, len);
	return ((len || **line) ? 1 : 0);
}
